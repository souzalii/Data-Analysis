...
plot(modelrf1) 
#from the graph, the best mtry = 3
...
# the graph almost linear, so we set min.node.size as 15
...
predbmw1 <- predict(object = modelRF,  
                           newdata = bmw1_test)

#Calculate RMSE
postResample(pred = predbmw1, obs = bmw1_test$price)

#predict compare to actual
modelRF_pa <- cbind(as.data.frame(predbmw1), bmw1_test$price)
